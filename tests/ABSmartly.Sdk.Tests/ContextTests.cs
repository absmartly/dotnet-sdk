...
public class ContextTests
{
    ...
    public void SetUp()
    {
        ...
        using var audienceStream = GetType().Assembly
            .GetManifestResourceStream("ABSmartly.Sdk.Tests.Resources.audience_context.json");
        _audienceData = deserializer.Deserialize(audienceStream);

        using var audienceStrictStream = GetType().Assembly
            .GetManifestResourceStream("ABSmartly.Sdk.Tests.Resources.audience_strict_context.json");
    ...
    public void TestConstructorSetsOverrides()
    {
        ...
        foreach (var (key, value) in overrides) context.GetOverride(key).Should().Be(value);
    }
    ...
    public void TestCallsEventLoggerWhenIncorrectData()
    {
        ...
        Mock.Get(_eventLogger)
            .Verify(x => x.HandleEvent(context, EventType.Error, "Context initialized with failed data."), Times.Once);
    }
    ...
    public void TestSetOverrideClearsAssignmentCache()
    {
        ...
        foreach (var (key, value) in overrides) context.GetTreatment(key).Should().Be(value);
    ...
    public void TestSetCustomAssignmentClearsAssignmentCache()
    {
        ...
        foreach (var (key, _) in customAssignments) context.GetTreatment(key).Should().Be(_expectedVariants[key]);
        context.PendingCount.Should().Be(customAssignments.Count);

        context.SetCustomAssignments(customAssignments);
        foreach (var (key, value) in customAssignments) context.GetTreatment(key).Should().Be(value);
    ...
    [Test]
    public void
        TestGetVariableValueQueuesExposureWithAudienceMismatchFalseAndControlVariantOnAudienceMismatchInStrictMode()
    {
        var context = CreateContext(_audienceStrictData);
    ...
    public void TestGetTreatment()
    {
        ...
        var expectedEvent = new PublishEvent
        {
            ...
            Exposures = new[]
            {
                ...
                Exposure(3, "exp_test_not_eligible", "user_id", 0, _clock.Millis(), true, false, false, false, false,
                    false),
    ...
    public void TestGetTreatmentReturnsOverrideVariant()
    {
        ...
        var expectedEvent = new PublishEvent
        {
            ...
            Exposures = new[]
            {
                ...
                Exposure(3, "exp_test_not_eligible", "user_id", 11, _clock.Millis(), false, true, true, false, false,
                    false),
                Exposure(4, "exp_test_fullon", "session_id", 13, _clock.Millis(), false, true, true, false, false,
                    false),
    ...
    public void TestGetTreatmentQueuesExposureOnce()
    {
        ...
        foreach (var experiment in _data.Experiments) context.GetTreatment(experiment.Name);
    ...
    public void TestTrack()
    {
        ...
        var expectedEvent = new PublishEvent
        {
            ...
            Goals = new[]
            {
                GoalAchievement("goal1", _clock.Millis(),
                    new Dictionary<string, object> { ["amount"] = 125, ["hours"] = 245 }),
    ...
    public void TestRefreshKeepsAssignmentCacheWhenNotChanged()
    {
        ...
        foreach (var experiment in _refreshedData.Experiments) context.GetTreatment(experiment.Name);
        context.GetTreatment("not_found");
