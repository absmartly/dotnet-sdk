...
public class ExprEvaluatorTests
{
    ...
    public void TestCompareObjects()
    {
        ...
        e.Compare(new List<object> { new List<object> { 1, 2 } }, new List<object> { new List<object> { 1, 2 } })
            .Should().Be(0);
    ...
    private static Dictionary<string, object> Dict(string op, object arg)
    {
        return new() { { op, arg } };
    }

    private static Dictionary<string, TValue> Dict<TValue>(params (string op, TValue arg)[] inputs)
    {
        var dict = new Dictionary<string, TValue>();
        foreach (var (op, arg) in inputs) dict[op] = arg;
